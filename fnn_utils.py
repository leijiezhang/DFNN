...
def fnn_centralized(self, x, y, max_n_rules, loss_functions: LossFunc, h_compute: HBase):
    ...
    for i in torch.arange(max_n_rules):
        ...
        h_compute.x = x
        h_compute.rules = ruls_tmp
        h_tmp = h_compute.comute_h()
        h_set.append(h_tmp)
...
def fnn_admm(d_train_data: List[Dataset], param_config: ParamConfig, w, h):
    ...
    param_mu = param_config.para_mu_current
    n_node = param_config.n_agents
