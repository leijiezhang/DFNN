...
loss_dlist = []
loss_admm_list = []
for j in torch.arange(30):
    loss_list_tmp = []
    loss_dlist_tmp = []
    loss_admm_list_tmp = []
    param_setting.n_rules = int(j + 1)
    for k in torch.arange(param_setting.kfolds):
        patition_strategy.set_current_folds(k)
        train_data, test_data = dataset.get_fold_data(k)
        ...
        loss_admm_list_tmp.append(errors)
        d_rules = RuleKmeans()
        ...
        loss_dlist_tmp.append(cfnn_loss)

    loss_list.append(loss_list_tmp)
    loss_dlist.append(loss_dlist_tmp)
    loss_admm_list.append(loss_admm_list_tmp)
