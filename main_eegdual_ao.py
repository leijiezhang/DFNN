...
from dfnn_run import dfnn_ite_rules_para_ao
...
param_config.config_parse('eegdual_config_ao')
...
for i in torch.arange(len(param_config.dataset_list)):
    ...
    loss_c_train_tsr, loss_c_test_tsr, loss_d_train_tsr, loss_d_test_tsr = \
        dfnn_ite_rules_para_ao(param_config, train_data, test_data)

    # save all the output
    acc_c_train_list.append(loss_c_train_tsr)
    acc_c_test_list.append(loss_c_test_tsr)
    acc_d_train_list.append(loss_d_train_tsr)
    acc_d_test_list.append(loss_d_test_tsr)

    # get the best performance
    acc_d_test_best = loss_d_test_tsr.max()
    pos_best = torch.where(loss_d_test_tsr == acc_d_test_best)
    acc_c_train_best = loss_c_train_tsr[pos_best[0][0], pos_best[1][0], pos_best[2][0]]
    acc_c_test_best = loss_c_test_tsr[pos_best[0][0], pos_best[1][0], pos_best[2][0]]
    acc_d_train_best = loss_d_train_tsr[pos_best[0][0], pos_best[1][0], pos_best[2][0]]
    acc_d_test_best = loss_d_test_tsr[pos_best[0][0], pos_best[1][0], pos_best[2][0]]
