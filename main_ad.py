...
for i in torch.arange(len(param_config.dataset_list)):
    ...
    acc_c_train_tsr, acc_c_test_tsr, acc_d_train_tsr, acc_d_test_tsr = \
        dfnn_rules_para_kfold_ao_s(param_config, dataset)
...
save_dict = dict()
save_dict["acc_c_train_list"] = torch.tensor(acc_c_train_list).numpy()
save_dict["acc_c_test_list"] = torch.tensor(acc_c_test_list).numpy()
save_dict["acc_d_train_list"] = torch.tensor(acc_d_train_list).numpy()
save_dict["acc_d_test_list"] = torch.tensor(acc_d_test_list).numpy()
torch.save(save_dict, data_save_file)

save_dict["acc_c_train_arr"] = torch.tensor(acc_c_train_arr).numpy()
save_dict["acc_c_test_arr"] = torch.tensor(acc_c_test_arr).numpy()
save_dict["acc_d_train_arr"] = torch.tensor(acc_d_train_arr).numpy()
save_dict["acc_d_test_arr"] = torch.tensor(acc_d_test_arr).numpy()
